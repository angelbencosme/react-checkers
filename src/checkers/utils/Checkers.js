...
export class Checkers {
    ...
    getMoves(boardState, coordinates, isKing = false, hasJumped = false) {
        ...
        const advanceRow = player === 'user' ? row - 1 : row + 1;
    ...
    movePiece(coordinates, state) {
        ...
        let hasJumped = null;
        let newMoves = [];
        let setCurrentPlayer = player === 'machine';
    ...
    shouldKing(movingPiece, coordinates) {
        ...
        return ((row === 1 && player === 'user') || (row === 8 && player === 'machine'));
    }
    ...
    evaluateWinner(boardState) {
        ...
        for (let coordinates in boardState) {
            ...
            if (boardState[coordinates].player === 'user') {
                ++userPieces;
                userMoves += moveCount;

            } else {
                ++machinePieces;
                machineMoves += moveCount;
            }
        }
